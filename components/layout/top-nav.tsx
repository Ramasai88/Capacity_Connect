...
import { Sparkles, Building2, LogOut, Database } from "lucide-react";
...
export function TopNav({
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border/80 bg-card/90 backdrop-blur-md px-6 shadow-xs transition-all">
      <div className="flex items-center gap-3">
        {displayOrgName && (
          <div className="flex items-center gap-2 rounded-lg bg-slate-100/80 px-2.5 py-1 border border-slate-200/60 shadow-2xs hover:bg-slate-100 transition-colors">
            <Building2 className="h-3.5 w-3.5 text-muted-foreground" />
            <span className="text-xs font-semibold text-foreground tracking-tight max-w-[280px] sm:max-w-md truncate">
              {displayOrgName}
            </span>
          </div>
        )}
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-3 group cursor-pointer">
          ...
          <div className="hidden sm:flex flex-col text-right">
            <span className="text-xs font-semibold text-foreground leading-tight group-hover:text-primary transition-colors">{userName}</span>
        ...
        <Button
          ...
          className="h-8 gap-1.5 text-xs text-muted-foreground hover:text-destructive hover:border-destructive/30 hover:bg-destructive/5 transition-all active:scale-95"
        >
