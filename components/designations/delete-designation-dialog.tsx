...
export function DeleteDesignationDialog({ designation, open, onOpenChange, onSuccess }: DeleteDesignationDialogProps) {
  const { deleteDesignation } = useDemoStore();
  ...
  async function handleDelete() {
    ...
    if (isDemoMode()) {
      const result = deleteDesignation(designation.id);
      if (!result.success) { setFeedback({ type: "error", message: result.error || "Cannot delete" }); return; }
      setFeedback({ type: "success", message: `"${designation.title}" has been deleted.` });
      setTimeout(() => { onOpenChange(false); setFeedback(null); onSuccess?.(); }, 1200);
    } else {
      try {
        await apiClient.designations.delete(designation.id);
        setFeedback({ type: "success", message: `"${designation.title}" has been deleted from PostgreSQL.` });
        setTimeout(() => { onOpenChange(false); setFeedback(null); onSuccess?.(); }, 1200);
      } catch (err: any) {
        setFeedback({ type: "error", message: err.message || "Failed to delete designation" });
      }
    }
  }
      ...
      <DialogContent className="max-w-md">
        <DialogHeader className="border-b pb-3">
          <div className="flex items-center gap-2">
            ...
            <div><DialogTitle className="text-lg font-bold text-destructive">Delete Designation</DialogTitle><DialogDescription className="text-xs">This action cannot be undone.</DialogDescription></div>
          </div>
        </DialogHeader>
        <div className="space-y-3 py-2">
          <div className="rounded-lg border bg-muted/40 p-3 text-sm">
            <div className="font-semibold">{designation?.title}</div>
            <div className="text-xs text-muted-foreground">{designation?.code} — {designation?.department}</div>
          </div>
          <div className="flex items-start gap-2 rounded-lg border border-amber-300 bg-amber-500/5 p-3 text-xs text-amber-900 dark:border-amber-900/50 dark:text-amber-300">
            ...
            <span>Deletion is blocked if any employees are currently assigned to this designation. Reassign them first.</span>
          </div>
          {feedback && (
        ...
        <DialogFooter className="border-t pt-3 flex sm:justify-between gap-2">
          ...
          <Button type="button" variant="destructive" size="sm" onClick={handleDelete} className="text-xs gap-1.5">
            <Trash2 className="h-3.5 w-3.5" />Delete Designation
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
